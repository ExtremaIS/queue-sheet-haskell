...
ifeq ($(MODE), cabal)
> @install -m 0755 \
>   "$(shell cabal list-bin $(CABAL_ARGS) queue-sheet)" \
>   "$(bindir)/queue-sheet"
else
...
install-doc: # install documentation to PREFIX/share/doc/queue-sheet-haskell
> @mkdir -p "$(docdir)"
...
> @pandoc -s -t man -o doc/queue-sheet.1 \
>   --variable header="queue-sheet Manual" \
